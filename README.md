...
As a shopper I want to be able to:  
   ...
   9. Easily select the size and quantity of the product, and add it to the shopping bag.  
...
### **D. Skeleton Plane**  
Wireframes were created using Figma to design the navigation and interface of the website. The wireframes were created only for desktops, which then will be transformed into high fidelity mock-ups in various device sizes (see Surface Plane).  
...
### **E. Surface Plane**  
High fidelity mock-ups were created using Figma as well for better and clearer visualization before coding, and also to check if the color scheme and images match and work great together. This has allowed the developer to concentrate solely on the development part (front end and back end), as the design has already been made with such details. The high-fidelity mock-ups were created in three different device sizes - desktop, tablet, and mobile.   
...
#### **Color Scheme**  
The overall color theme of the website was natural earthy colors, which have relaxed and comforting influences and encourage feelings of warmth and calmness. Using [Coolors](https://coolors.co), I started with beige (#E8DCD5) and brown (#644536), and generated one more color that compliments them both - the result was a salmon color (#B2675E). I also found a hero image that fits the overall theme and colors accordingly.   
...
- In the wireframes and mockups, there was a subscription to the newsletter box. I decided not to include this subscription to the newsletter functionality on the final product due to time constraints.  
...
- Minor things like buttons colors and image differences were purely design choices that were taken during the website development.  
...
The database schema was designed using [drawsql](https://drawsql.app/). The database was first managed using SQLite during the development process, then Postgres in production with Heroku.   
...
- **Product model**   
   ...
   - SKU field: alphanumeric code assigned for a product.  
   ...
   - Rating: the rating of the product. Each product has an initial rating value given by the retailer based on its quality from the manufacturer. The default value is 3. Each time a user gives a rating to the product, it calculates the average and updates this field (read more about this in Feature section - Add Review).  
...
- **Favorites model**  
   - Stores list of favorite products of a registered user.  
   - User: the user that created the favorites list, a foreign key from the User model.  
   - Products field: the list of products.   
   - Because there would be a many-to-many relationship between favorites and products, i.e. one product can be listed in multiple (users) favorites, and one favorite can have multiple products, I decided to define a through/ intermediary model between the two models ([source from Django documentation](https://docs.djangoproject.com/en/4.0/ref/models/fields/#django.db.models.ManyToManyField.through_fields)). The through model named  **FavoritesItem model**, which stores each product in one table with favorites as a foreign key.  

- **Topic model**  
  - Stores the topics of the contact form.  
  - Subject field: a one-word name for database purposes.  
  - Text field: a longer name for display on the contact form.  
  - I decided to make this into a separate model instead of a choice field because I wanted to define separate naming fields for database and display, and for future flexibility in adding/ editing/ removing a topic.   

- **ContactForm model**  
  - Stores the contact form filled by the user.  
  - Name field: the name of the user.  
  - Email field: the email of the user.  
  - Topic field: the topic of the message, a foreign key from the Topic model.  
...
- **Navbar** (on all pages)   
   ...
   - The logo is placed at the top center of the Navbar, creating a symmetrical visual between left and right, and links back to the Home page.  
   ...
   - Favorites and Shopping Bag links are placed on the Navbar to give quick access from any page the user is currently at.   
...
- **Footer** (on all pages)  
   ...
   The Footer uses the same color as the Navbar and provides all navigation links, the social media links of the store as well (that opens in a new tab), and the logo that leads back to the Home page. Here we also have links to the Articles, the FAQ page, and the Contact page. Since the main business of Shoes And More is selling shoes and accessories, these three pages although still important, are not the main pages of the site.  Therefore the links to these pages are only available on the footer, which can be found at the bottom of every page nevertheless.  
...
- The best sellers section gives information to users on which products have been purchased the most, which indicates the most popular products. The products are displayed in a multi-item carousel of 8 products. If a product is purchased, the quantity of purchase will accumulate for each product. To get the 'best sellers' products, all products will be sorted according to their total purchased quantity from highest to lowest, and the 8 products with the highest quantity are displayed in this section.   
- The final sale section provides direct navigation to the sale products.   
- The bags and accessories section provides direct navigation to the accessories products.  
...
- The Products page starts with breadcrumb navigation that indicates the current page’s location within a navigational hierarchy. For example, if the user chose the Women Sneakers, then the breadcrumb will show Home > Women > Sneakers.  
- Below the breadcrumb, there's a 'Final Sale' banner, as normally, users are interested in checking out the sale items first. If users click the shop now button, it will filter the products to display only the sale items. This Sale banner is not displayed on mobiles to save space.  
- The side Navigation feature (only on desktop) helps users to quickly navigate through the categories and subcategories and will filter the products to display only the categories and subcategories of choice. On smaller devices, users can click the hamburger icon to navigate.   
- The Side Navigation can't be applied on mobiles and tablets due to the limitation of space. To replace it, a dropdown button is available on mobiles and tablets in order to provide the same functionality to navigate throughout subcategories.  
...
- On the top right of each product's image, there's a heart icon that turns red when the user hovers over it. By clicking this icon, a registered user can add the product to their favorites list. The heart icon will then turns red to indicate that the product has been listed in the user's favorites.  
...
- The Individual Product page starts with breadcrumb navigation that indicates the current page’s location within a navigational hierarchy.  
...
- Under the product information, there's a review section that lists all the reviews for the product. If a user has written a review for the product before, the buttons to edit and delete show up below it. A registered user is able to edit or delete their own review.   
   ...
   If the user clicks the Edit button, they will be directed to the Edit Review page [see Edit Review page](https://github.com/dissyulina/shoesandmore#11-add-edit-review-page).   
...
- The Favorites page features all products that the user has been saved as their favorites. The products are displayed in resemblance with the products page, using the product's image, name, price, and rating. They are displayed in 4 columns on a large screen, 3 columns on a medium screen, and 2 columns on a small screen.  
- The heart icon on the top right of the product's image is replaced by a trash icon, which indicates its function to remove the product from favorites.  
- Since favorites are a list of products that the user is interested in and willing to buy, below each product, there is an option to choose the size and an Add to Bag button. This allows users to add the product to their shopping bag directly and easily, without going to the individual product page.   

#### **5. Bag Page**  
[See the image of Bag page here](readme-testing-files/readme/bag-page.png)  
- The Bag page features all products that the user has been added to the shopping bag, and the total price that the user has to pay (if the user decides to continue). On this page, users can adjust the products' quantity, or remove the products from the bag.  
- It displays the products' information such as product's image, name, SKU, price, size of choice, and quantity of choice.  
...
- The Add Review page features a form to submit ratings and reviews for the specified product. Users can give a rating (range from 1 to 5), and/or a review of the product. The rating is required (users can't submit the form if the rating is empty), while the review text input is not.  
...
- The Articles page features all relevant articles about shoes and accessories. This provides better customer experiences besides basic purchasing functionality and hopefully will increase users' engagement with the store.   
...
- The Product Management page features a form that functions to add a product to the store. On this form, the admin can fill out the category, subcategory, SKU, name, has sizes (true or false), price, rating, and image.  
...
- The Edit Product page's design and layout are the same as Add Product page to give a sense of consistency and minimal learning curve for the admin.  
...
Due to limited resources (time constraints, skill of the developer at the moment, and other reasons), some features couldn't be implemented now. There are so many potentials to develop this e-commerce website in the next development phase, adding more features and functionality. These features would be a great addition to the web app:  
1. Adding more payment methods.  
   While there are many methods of payments that can be implemented, because of time constraints, I could only add payments with cards. In the next development phase, more payment methods should be added, such as bank account debit, PayPal, Apple pay, etc, so that it would resemble a real e-commerce / online store website more.   
2. Adding an option to sign in with social media.   
   Social media logins make registration faster and easier and some users would prefer that. Websites can also gather profile- or social data for further analysis purposes. Social login also gives instant demographic and psychographic data about the customers, which can be used for better segmentation, website design, products, and so on.   
3. Adding stock management for the products.  
   Right now there is no stock field on the product model, so it's based on assumption that the logistic management is taken care of somewhere else (not on the website). It would be a great addition to the project to add stock management functionality, where the number of products available is also a field in the product table. When the number reaches a certain amount, it alerts a warning to restock the amount. If the number reaches 0, the product would not be displayed on the website until it's restocked.  
...
1. **ISSUE** : For products that are in the favorites list of the user (that's currently logged in), the heart icon on the top left of the product image becomes red. I had difficulties in trying to implement this feature. I tried to check if the product is listed in the queryset of the users' favorites by defining this:   
   ...
   **SOLUTION** : The Code Institute's Tutor has helped me to figure out and to understand how to check if an instance exists in a query set, and how to define the correct query set. After going through the code, the solution was fairly simple:   
   In views.py:   
   ```
   products = Product.objects.all()  
...
2. **ISSUE** : The second problem was very similar to the first one. For every registered user, on their profile page, there's a review section. This review section lists all products that the user has bought before, and are waiting to be reviewed. Once the user gives a review of a product, the product will disappear from this section. Therefore, for this section, I wanted to display only the products that are in the user's order history, and are not listed in the reviews database. This problem was rather complicated because you have to check in multiple tables, and the process is not straightforward. Based on my understanding, this is what came to my mind.  
   In views.py:   
   ```
...
3. **ISSUE** : After a user gives a rating to a product, I wanted to update the product's rating by recalculating the average of the ratings. Each product has an initial rating given by the admin (the default rating is 3, but the admin can input another number). Here's the calculation example:   
...
In this way, I would have to maintain two variables - the initial product rating, and the updated rating. I would have to get these variables and all ratings for this product to be able to recalculate the average each time a new rating is submitted.   
...
* From [Pexels](https://www.pexels.com/)  
   ...
   - Image on the Women collection on Homepage: by Dziana Hasanbekava  
   - Image on the Men collection on Homepage: by Yo Yo  
   - Image on the Kids collection on Homepage: by Victoria Borodinova  
   - Image on the Sale collection on Homepage: by Cliff Booth  
   - Image on the Bags and accessories collection on Homepage:  by Socialcut  
   - Image on the article 'Best Paint Markers to Use for Bag and Shoe Designs' on Articles page: by Mister Mister  
   - Image on the article 'Ways to Make Your All-Weather Boots More Fashionable' on Articles page: by Pavel Danilyuk  
   - Image on the article 'Mix and Match Your Style for Spring!' on Articles page: by Engin Akyurt  
   - Image on the article 'These Forgotten $45 Sneakers From the ’90s Are Making a Major Comeback' on Articles page: by Karolina Grabowska  

* From [Unsplash](https://unsplash.com/)  
   - Image on the Sneakers collection on Homepage : by Samuel Lopes.  
   - Image on the article 'The Ultimate Guide to Spring Style for Men' on Articles page: by Rayul  

* From [Freepik](https://www.freepik.com/)  
   - The Error page images - 404 page, 403 page, and 500 page: by Storyset  
